...
def vector_to_param(model, vector):
...
class ERM(torch.optim.Optimizer):
    def __init__(self, params, base_optimizer, **kwargs):
        default = dict(**kwargs)
        super(ERM, self).__init__(params, default)
...
class SAM:
    """
    SAM minimizer for FedGamma algorithm
    Implements Sharpness-Aware Minimization with ascent_step functionality
    """

    def __init__(self, optimizer, model, rho):
    ...
    def ascent_step(self):
        """Perform ascent step to climb to local maximum"""
    ...
    def get_model_gradients(self):
        """Extract model gradients as a flattened vector"""
        grads = []
        for p in self.model.parameters():
            if p.grad is not None:
                grads.append(p.grad.reshape(-1))
            else:
                grads.append(torch.zeros_like(p.data.reshape(-1), device=p.device))
        self.model_gradients = torch.cat(grads)
        return self.model_gradients
...
class ESAM(torch.optim.Optimizer):
    ...
    def step(self, alpha=1.):
        ...
        loss = alpha * loss_func(predictions2, labels.long())
        self.zero_grad()
        loss.backward()
        self.second_step()
