...
def grad_norm(device, model, loader, criterion, lp: int = 2) -> float:
...
class ModelWrapper:
    def __init__(self, model, dataloaders, criterion, use_cuda=True):
    ...
    def compute_loss(self, ascent_stats=False):
        self.zero_grad()
        ...
        for inputs, targets in self.dataloader:
            ...
            if ascent_stats:
                loss = -loss * (inputs.size(0) / len(self.dataloader[phase].dataset))
                loss.backward()
        return total_loss / total_n, None  # 需要精度时可自行计算
    ...
    # 可选：state_dict 透传，便于你的 low_pass 保存/恢复
    def state_dict(self):
...
def low_pass(model, dataloader, criterion, sigma, mcmc_itr):
    out = 0.0
    model_func = ModelWrapper(model, dataloader, criterion)
    state_dict = model_func.state_dict()
    with torch.no_grad():
    ...
    model_func.load_state_dict(state_dict)
    return out/mcmc_itr
